...
% ----------------------------------------------------------------------
function [mrx,echo_rx,echo_int]=sim_cpmg_ideal_tv(sp,pp)
...
% This loop is the most computationally expensive part of the code
% parfor has too much overhead and does not help here
echo_rx=zeros(NE,nacq); % Size: [NE,nacq]
echo_int=zeros(NE,1); % Size: [NE,1]
for i=1:NE
    % Add field offsets to isochromats before calculating echoes
    echo_rx(i,:)=(exp(1i*w_0t(i)*tvect_rep).*isoc)*mrx(:,i);
    echo_int(i)=trapz(tvect,echo_rx(i,:)); % Estimate echo integrals
end
mrx=mrx';
...
if sp.plt_output % Plot selected outputs
    ...
    subplot(1,2,1);
    imagesc(sp.del_w,tseq*1e3,real(mrx)); colorbar;
    set(gca,'FontSize',14); set(gca,'FontWeight','Bold');
    xlabel('\Delta\omega_0/\omega_{1n}'); ylabel('Sequence time (ms)');
    title('Re(M_{rx}(\omega))');
    subplot(1,2,2);
    imagesc(sp.del_w,tseq*1e3,imag(mrx)); colorbar;
    set(gca,'FontSize',14); set(gca,'FontWeight','Bold');
    xlabel('\Delta\omega_0/\omega_{1n}'); ylabel('Sequence time (ms)');
    title('Im(M_{rx}(\omega))');
    
    figure(2); % Echoes
    subplot(1,2,1);
    imagesc(tvect,tseq*1e3,real(echo_rx)); colorbar;
    set(gca,'FontSize',14); set(gca,'FontWeight','Bold');
    xlabel('t_{acq} (norm)'); ylabel('Sequence time (ms)');
    title('Re(s_{rx}(t))');
    subplot(1,2,2);
    imagesc(tvect,tseq*1e3,imag(echo_rx)); colorbar;
    set(gca,'FontSize',14); set(gca,'FontWeight','Bold');
    xlabel('t_{acq} (norm)'); ylabel('Sequence time (ms)');
    title('Im(s_{rx}(t))');
    
    figure(3); % Echo integrals
    subplot(2,1,1);
    stairs(tseq*1e3,w_0t,'LineWidth',1);
    set(gca,'FontSize',14); set(gca,'FontWeight','Bold');
    ylabel('\Delta\omega_{0}(t)/\omega_{1n}');
    subplot(2,1,2); cla;
    %plot(tseq*1e3,real(echo_int),'LineWidth',1,'Marker','o'); hold on;
    %plot(tseq*1e3,imag(echo_int),'LineWidth',1,'Marker','o');
    plot(tseq*1e3,real(echo_int),'LineWidth',1); hold on;
    plot(tseq*1e3,imag(echo_int),'LineWidth',1);
    set(gca,'FontSize',14); set(gca,'FontWeight','Bold');
    xlabel('Sequence time (ms)'); ylabel('s_{int}(t)');
end