...
function [macq,mrx,pnoise,f,echo_rx,tvect]=calc_macq_matched_probe_grad(sp,pp)

del_w=sp.del_w; % Offset frequency vector
del_wg=sp.del_wg; % Change in offset frequency vector
...
tp=pp.tp; phi=pp.phi; amp=pp.amp; acq=pp.acq; grad=pp.grad; % Real time
tp_curr=[]; phi_curr=[]; amp_curr=[]; acq_curr=[]; grad_curr=[]; % Normalized time
...
for i=1:length(tp)
    ...
    if state_rf==1 && amp(i)==0 % End of RF pulse
        ...
        acq_exc=zeros(1,length(texc)); grad_exc=zeros(1,length(texc));
        % Note: for convenience, assume no gradient during an RF pulse
        ...
        grad_curr = [grad_curr grad_exc grad(i)];
    end
...
if sp.plt_sequence
    ...
    subplot(3,1,2);
    stairs(tplt*1e3,[grad_curr grad_curr(end)],'LineWidth',1);
...
params.grad=grad_curr; params.len_acq=tacq; 
params.del_w=del_w; params.del_wg=del_wg;
params.w_1=w_1;
