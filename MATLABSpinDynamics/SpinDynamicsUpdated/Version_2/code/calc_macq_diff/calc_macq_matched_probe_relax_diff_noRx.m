...
function [macq,mrx,pnoise,echo_rx,tvect]=calc_macq_matched_probe_relax_diff_noRx(sp,pp)
...
phi=pp.phi;
amp=pp.amp;
acq=pp.acq; % Real time
tp_curr=[]; 
phi_curr=[];
amp_curr=[];
acq_curr=[]; % Normalized time
state_rf=0; 
state_rf_curr=0;
...
for i=1:length(tp)
    ...
    if state_rf==0 && amp(i)==0 % Time delay, copy to new structure
        tp_curr = [tp_curr (pi/2)*tp(i)/T_90];
        phi_curr = [phi_curr phi(i)];
    ...
    if state_rf==1 && amp(i)==0 % End of RF pulse
        ...
        sp.tf1=tf1; 
        sp.tf2=tf2; % Save Rx transfer functions
...
params.mth=sp.mth;
params.D = sp.D;
params.gamma = sp.gamma;
params.grad = sp.grad;
params.pul = pp.pul;
params.Delta = sp.Delta;
