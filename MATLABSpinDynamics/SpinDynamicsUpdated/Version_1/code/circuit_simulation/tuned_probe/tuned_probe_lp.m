...
function [tvect2,Icr2,tvect,Icr] = tuned_probe_lp(sp,pp)
...
for j=1:length(np)
    ...
    if amp(j)==0
        ...
        % +w component
        Icd1=(amp(j)/Rs(j))*exp(1i*phi_eff)*(exp(1i*w*tv)+((lambda(2)-1i*w)*exp(lambda(1)*tv)-...
            (lambda(1)-1i*w)*exp(lambda(2)*tv))/(2*alpha))/(wn^2-w^2+2*1i*gamma*w);
        % -w component
        Icd2=(amp(j)/Rs(j))*exp(-1i*phi_eff)*(exp(-1i*w*tv)+((lambda(2)+1i*w)*exp(lambda(1)*tv)-...
            (lambda(1)+1i*w)*exp(lambda(2)*tv))/(2*alpha))/(wn^2-w^2-2*1i*gamma*w);
        Icd=0.5*(Icd1+Icd2);
        
        % +w component
        dIcd1=(amp(j)/Rs(j))*exp(1i*phi_eff)*(1i*w*exp(1i*w*tve)+(lambda(1)*(lambda(2)-1i*w)*exp(lambda(1)*tve)-...
            lambda(2)*(lambda(1)-1i*w)*exp(lambda(2)*tve))/(2*alpha))/(wn^2-w^2+2*1i*gamma*w);
        % -w component
        dIcd2=(amp(j)/Rs(j))*exp(-1i*phi_eff)*(-1i*w*exp(-1i*w*tve)+(lambda(1)*(lambda(2)+1i*w)*exp(lambda(1)*tve)-...
            lambda(2)*(lambda(1)+1i*w)*exp(lambda(2)*tve))/(2*alpha))/(wn^2-w^2-2*1i*gamma*w);
...
if sp.plt_tx
    figure(1); clf;
    % Rotating frame
    plot(tvect2*1e6,real(Icr2));  hold on;
    plot(tvect2*1e6,imag(Icr2),'r-');
    whiteBg;
    setSize;
    font;
...
%      export_fig D:\Dropbox\TuneMatchJMR\Figures\Updated\TxTune.pdf
%     plot(tvect2*1e6,abs(Icr2),'k-');
    ...
    setSize;
    font;
%     export_fig D:\Dropbox\TuneMatchJMR\Figures\Updated\TxTuneDemod.pdf
end
