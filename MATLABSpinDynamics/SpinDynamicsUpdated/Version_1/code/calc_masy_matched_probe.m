...
function [mrx,echo_rx,tvect,SNR]=calc_masy_matched_probe(sp,pp)
...
pp_curr=pp; tdel=2*T_90; % Added delay
pp_curr.tp=[pp.texc tdel];
pp_curr.phi=[pp.pexc 0]; pp_curr.amp=[pp.aexc 0];
...
tdeln=(pi/2)*tdel/T_90; tcorrn=(pi/2)*pp.tcorr/T_90;
...
texc=[texc -tdeln tcorrn]; pexc=[pexc 0 0]; aexc=[aexc 0 0];
...
sp.plt_rx=0; 
[mrx,SNR] = matched_probe_rx(sp,pp,masy,tf1,tf2);

% Calculate time-domain echo
[echo_rx,tvect]=calc_time_domain_echo(mrx,sp.del_w,sp.plt_echo);