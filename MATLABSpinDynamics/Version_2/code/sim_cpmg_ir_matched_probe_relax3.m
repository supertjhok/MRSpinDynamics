...
function [echo_int_all]=sim_cpmg_ir_matched_probe_relax3(NE,TE,tauvect,T1,T2)
...
parfor j=1:ntau % Parallelize for speed
    ...
    % Run PAP phase cycle
    [~,mrx1,~,~,~,~]=calc_macq_matched_probe_relax3(sp,ppc1);
    [~,mrx2,~,~,~,~]=calc_macq_matched_probe_relax3(sp,ppc2);
