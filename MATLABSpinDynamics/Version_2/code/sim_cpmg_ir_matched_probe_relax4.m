...
function [echo_int_all echo_asymp_all]=sim_cpmg_ir_matched_probe_relax4(NE,TE,tauvect,T1,T2)
...
texc=[127*pi/180 TE/2-pi/2 127*pi/180 127*pi/180]; 
aexc=[1 0 1 1];
pexc1=[1 0 1 3]; % pexc is pulse type
acq_exc=[0 0 0 0]; gexc=[0 0 0 0]; % gexc is the gradient

%  Excitation pulse 2, including timing correction
pexc2=[1 0 1 1]; % pexc is pulse type

% Refocusing cycles
nref=4; % Segments in refocusing cycle
...
for i=1:NE
    tref((i-1)*nref+1:i*nref)=[tfp 127*pi/180 127*pi/180 tfp];
    pref((i-1)*nref+1:i*nref)=[0 1 3 0]; % Pulse type
    aref((i-1)*nref+1:i*nref)=[0 1 1 0];
    acq_ref((i-1)*nref+1:i*nref)=[0 0 0 1];
    gref((i-1)*nref+1:i*nref)=[0 0 0 0]; % Gradient
end
...
%Encoding period with composite pulses to measure T1 from echoshape
tenc=[127*pi/180 pi/2 127*pi/180 (pi/2)*tauvect(1)/T_90];
penc=[pi/2 0 0 0]; 
penc2=[pi/2 0 pi 0];
aenc=[1 0 1 0];
genc=[0 0 0 0]; 
