...
tfpa = 2*tfp;
    ...
    tref=[tfp pi tfp tfp];
    pref=[0 3 0 0]; % Pulse type
    aref=[0 1 0 0];
    acq_ref=[0 0 1 1];
    gref=[0 0 0 0]; % Gradient
...
for i=1:NE
    ...
    plot(((i-1)*(max((tvect*T_90/(pi/2)+i*TE))-min((tvect*T_90/(pi/2)+i*TE)))+((tvect*T_90/(pi/2)+TE)))*1e3,real(echo_rx(i,:)),'b-'); hold on;
    plot(((i-1)*(max((tvect(1,:)*T_90/(pi/2)+i*TE))-min((tvect*T_90/(pi/2)+i*TE)))+((tvect*T_90/(pi/2)+TE)))*1e3,imag(echo_rx(i,:)),'r-'); 
end
