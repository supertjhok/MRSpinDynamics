...
function compare_cpmg_results_ideal_v0crit(file,pulse_num,NE)
...
% Define nominal field fluctuation waveform
pp.NE=NE; % Number of echoes
w_1n=(pi/2)/pp.T_90; % Nominal nutation frequency
% Constant
% B_0t_nom=zeros(1,NE);
% Linear ramp
% B_0t_nom=(w_1n/sp.gamma)*linspace(0,1,NE);
% Triangle wave
% B_0t_nom=(w_1n/sp.gamma)*[linspace(0,1,NE/4) linspace(1,-1,NE/2) linspace(-1,0,NE/4)];
% B_0t_nom=(w_1n/sp.gamma)*[linspace(0,1,NE/2) linspace(1,0,NE/2)];
% Sinusoid
B_0t_nom=(w_1n/sp.gamma)*sin(2*pi*0.5*linspace(0,1,NE));
...
% Modify offset fields if necessary
sp.maxoffs_y=1; sp.ny=5e3;
