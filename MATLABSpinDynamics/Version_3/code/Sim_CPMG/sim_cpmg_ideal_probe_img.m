...
% Simulate a 2D CPMG image with ideal probe dynamics.
%
% Signature
...
%
% Dependencies
%   set_params_ideal, create_fields_single_sided, calc_rotation_matrix,
%   sim_spin_dynamics_arb10 or related arbitrary-pulse kernels.
%
% Notes
...
% Exc pulse y (phase = pi/2)
pp_in.tp=pi/2; pp_in.phi=pi/2; pp_in.amp=1;
Rtot{1}=calc_rotation_matrix(sp,pp_in);

% Exc pulse -y (phase = 3*pi/2)
pp_in.tp=pi/2; pp_in.phi=3*pi/2;
Rtot{2}=calc_rotation_matrix(sp,pp_in);

% Ref pulse x (phase = 0)
pp_in.tp=pi; pp_in.phi=0;
Rtot{3}=calc_rotation_matrix(sp,pp_in);

% Ref pulse y (phase = pi/2)
pp_in.tp=pi; pp_in.phi=pi/2;
...
parfor i=1:px % Parallelize for speed
    ...
    for j=1:pz
        ...
        ppc.pul=pul1; mrx1=calc_macq_ideal_probe_relax4(spc,ppc);
        ppc.pul=pul2; mrx2=calc_macq_ideal_probe_relax4(spc,ppc);
        mrx_x=(mrx1-mrx2); % Apply phase cycle
        
        % Run PAP phase cycle: (3-4)
        ppc.pul=pul3; mrx3=calc_macq_ideal_probe_relax4(spc,ppc);
        ppc.pul=pul4; mrx4=calc_macq_ideal_probe_relax4(spc,ppc);
