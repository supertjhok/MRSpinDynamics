...
function opt_ref_pulse_ideal_tv_repeat(ref_len,tE,file)

% Number of iteration runs (should be integer multiple of parpool size)
numiter=16;

[sp,pp]=set_params_ideal_tv; % Set parameters
...
w_0max=1.5; % Maximum normalized frequency offset
% Constant
% sp.B_0t=zeros(1,NE);
% Linear ramp
...
params.tvect=linspace(-tacq/2,tacq/2,nacq)'; % Acquisition vector, size: [nacq,1]
...
parfor count = 1:numiter
    ...
    % Run optimization
    [out]=opt_ref_pulse_ideal_tv(params_curr,sp,pp);
