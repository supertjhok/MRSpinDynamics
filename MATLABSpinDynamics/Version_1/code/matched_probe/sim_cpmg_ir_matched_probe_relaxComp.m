...
function [echo_int_all,echo_asymp_all]=sim_cpmg_ir_matched_probe_relaxComp(NE,TE,tauvect,T1,T2)
...
tacq=(2*pi)*pp.tacq/T_90; % Normalized acquisition window length
...
%     % Encoding period
%     tenc=[ppc.T_180*0.705 ppc.T_90 ppc.T_180*0.705 tauvect(j)];
%     penc=[0 0 pi/2 0]; 
%     penc2 = [0 0 pi/2+pi 0];
%     aenc=[1 0 1 0]; 
%     acq_enc=[0 0 0 0];
...
%     texc=[ppc.T_180*0.705 TE/2-ppc.T_90 ppc.T_180*0.705 ppc.T_180*0.705];
%     pexc=[0 0 0 pi]; 
%     aexc=[1 0 1 1]; 
%     acq_exc=[0 0 0 0];
 ...
 % Encoding period
    ...
    acq_ref=tref;
    for i=1:NE
%         tref((i-1)*numComp+1:i*numComp)=[((TE-ppc.T_180*0.705)/2) ppc.T_180*0.705 ppc.T_180*0.705 ((TE-ppc.T_180*0.705)/2)];
%         pref((i-1)*numComp+1:i*numComp)=[0 0 pi 0];
%         aref((i-1)*numComp+1:i*numComp)=[0 1 1 0];
%         acq_ref((i-1)*numComp+1:i*numComp)=[0 0 0 1];
