...
% grad -> (value, length)
% -----------------------------------------------------------------------
% Sequence: (pi/2)x - (pi)y
function [echo_rx,tvect,SNR]=sim_cpmg_matched_probe_grady(NE,TE,T1,T2,grad)
...
texc=[1 -2/pi]*pp.T_90; 
pexc=[0 0]; aexc=[1 0]; acq_exc=[0 0]; grad_exc=[0 0];
