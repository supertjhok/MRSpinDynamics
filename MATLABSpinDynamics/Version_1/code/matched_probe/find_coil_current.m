...
L=sp.L; Q=sp.Q; f0=sp.f0; Rs=sp.Rs;
C1=sp.C1; C2=sp.C2;
fin=sp.fin;
    
% Calculate normalization parameters
n=C1/C2; Z0=sqrt(L/C1); wp=1/sqrt(L*C1);
wn=(2*pi*fin)/wp; % Normalized input frequency
Rc=(2*pi*f0*L)/Q; % Coil series resistance (Ohms)

c1=n*Z0/Rs; c2=(Rc/Rs)*(n+1)+1;
c3=Rc/Z0+(Z0/Rs)*(n+1); Vs0=2*sqrt(Rc/Rs);

% Read pulse sequence parameters
tp=wp*pp.tp; phi=pp.phi; amp=pp.amp;
N=pp.N; % quantization step = N x input RF frequency
psi=pp.psi;

% Simulation frequency range
w1=pi/(2*pp.T_90); wv=(2*pi*f0+w1*sp.del_w)/wp;
...
s=tf('s'); tf0=Vs0/(s^3+c3*s^2+c2*s+c1);
...
for i=1:length(tp)
    ...
    [~,ycos(ind1:ind2,:)] = ode45(@(t,y) odefcn_cos(t,y,ode_params), tvec(ind1:ind2), ycos0, opts);
    [~,ysin(ind1:ind2,:)] = ode45(@(t,y) odefcn_sin(t,y,ode_params), tvec(ind1:ind2), ysin0, opts);
...
function dydt = odefcn_cos(t,y,params)
...
function dydt = odefcn_sin(t,y,params)
