...
function [masy,SNR] = plot_hardExc_octRef_tuned_probe_lp(file,pulseNum)

[sp,pp]=set_params_lp_tapped;
tmp = load(file);
results = tmp.results;
sp = results{pulseNum,6};
pp = results{pulseNum,7};
sp.cycle=2*pi/sp.w0; % Length of one RF cycle
params = results{pulseNum,5};
