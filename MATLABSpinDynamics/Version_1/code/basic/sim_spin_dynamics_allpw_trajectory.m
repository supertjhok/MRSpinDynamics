...
% powdb = pulse power in dB (T_90 = 0dB)
...
function [echo,tvect,mxyz]=sim_spin_dynamics_allpw_trajectory(T_90,tp,phi,powdb,tf,T1,T2,offs)
...
numpts=length(offs); % number of spin vectors to simulate
...
amp=10.^(powdb/20); % relative B1 amplitude
w1=pi/(2*T_90);
del_w=w1*offs;

% Calculate spin dynamics (no diffusion)
mvect=zeros(3,1,numpts); % Magnetization vectors (coherence form)
mxyz=zeros(3,1,numpts); % Magnetization vectors (xyz form)
mlong=zeros(3,1,numpts);

mvect(1,1,:)=m0*ones(1,1,numpts); % Initial mag vectors are along z-axis

num_pulses=length(phi);
xvect=zeros(numpts,num_pulses+length(find(tf>0))+1); yvect=xvect; zvect=xvect; % Trajectories
count=1;

% Map from coherences to xyz
mxyz(1,1,:)=(mvect(map_pi(1),1,:)+mvect(map_pi(-1),1,:))/2;
mxyz(2,1,:)=(mvect(map_pi(1),1,:)-mvect(map_pi(-1),1,:))/(2*i);
mxyz(3,1,:)=mvect(map_pi(0),1,:);

% Begin alculating trajectory
for k=1:numpts
    xvect(k,count)=mxyz(1,1,k);
    yvect(k,count)=mxyz(2,1,k);
    zvect(k,count)=mxyz(3,1,k);
end
...
for j=1:num_pulses
    ...
    for k=1:numpts
        xvect(k,j+count)=mxyz(1,1,k);
        yvect(k,j+count)=mxyz(2,1,k);
        zvect(k,j+count)=mxyz(3,1,k);
    end
...
figure(1);
plot(tvect*1e6,abs(echo),'b-','LineWidth',2);
set(gca,'FontSize',14);

% Plot trajectories
figure(2);
for k=1:numpts
    plot3(real(xvect(k,:)),real(yvect(k,:)),real(zvect(k,:)),'k-','LineWidth',2);
    hold on;
end
set(gca,'FontSize',14);
...
[x,y,z]=sphere(10);
h=surf(x,y,z);
set(h,'FaceColor','none','EdgeColor','blue','LineStyle','--');
grid;
