...
function [macq,m_z,del_w]=sim_spin_dynamics_arba_relax2(tp,phi,amp,acq,len_acq,T1,T2)

cm=map_pi(-1); c0=map_pi(0);

% Uniform distribution of del_w0 (uniform gradient)
maxoffs=40;
numpts=2e4+1; % number of spin vectors to simulate
...
macq=zeros(nacq,numpts); m_z=macq;
trans=zeros(1,numpts); trans_z=trans;
...
for j=1:num_pulses
    ...
    if acq(j) % Acquire spectrum
        acq_cnt=acq_cnt+1;
        for k=1:numpts
            trans(k)=mvect(cm,1,k); % Only -1 coherence is visible
            trans_z(k)=mvect(c0,1,k); % 0 coherence / z-magnetization
        end
        
        fy = conv(trans,window);
        macq(acq_cnt,:) = fy(((numpts+1)/2:3*(numpts-1)/2+1));
        
        fz = conv(trans_z,window);
        m_z(acq_cnt,:) = fz(((numpts+1)/2:3*(numpts-1)/2+1));
    end
end
