% Plot normalized SNR for OCT excitation pulses with variable segment lengths

function plot_normalized_snr(datafile,clr)
...
rms=zeros(1,nump); Tseg=rms;
for i=1:nump
    tmp=results{i,1}; Tseg(i)=tmp(1);
    rms(i)=results{i,4};
end

figure(1);
plot(Tseg/T_180,(rms/rect_rms).^2,clr); hold on;
ylabel('Normalized SNR (power units)')
xlabel('Segment length (\times T_{180})')