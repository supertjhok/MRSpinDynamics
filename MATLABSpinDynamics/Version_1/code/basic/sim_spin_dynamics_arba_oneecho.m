...
% Calculate single refocusing cycle
% ------------------------------------
...
function [mecho,mvect]=sim_spin_dynamics_arba_oneecho(tref,pref,aref,del_w,t_acq,mvect)

numpts=length(del_w);
my=zeros(1,numpts); mecho=zeros(1,numpts);
...
num_ref=length(pref); % Number of elements in each refocusing cycle
for j=1:num_ref
    if aref(j)>0
        w1=aref(j);
        Omega=sqrt(w1.^2+del_w.^2);
        
        mat=calc_matrix_elements(del_w,w1,Omega,tref(j),pref(j)); % RF pulses
    else
        mat=calc_fp_matrix_elements(del_w,tref(j)); % Free precession
    end
...
fy = conv(my,window);
mecho = fy(((numpts+1)/2:3*(numpts-1)/2+1));
