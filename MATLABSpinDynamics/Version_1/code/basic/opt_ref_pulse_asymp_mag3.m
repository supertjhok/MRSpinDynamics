...
function [out]=opt_ref_pulse_asymp_mag3(params)
...
trefc=[tfp; tref; flipud(tref); tfp]; % Create refocusing cycle
prefc=[0; pref; -flipud(pref); 0];
arefc=[0; aref; aref; 0];
...
numpts=length(del_w);
trefc=[tfp; params; flipud(params); tfp]; % Create refocusing cycle
prefc=[0; pref; -flipud(pref); 0];
arefc=[0; aref; aref; 0];
[neff]=calc_rot_axis_arba2(trefc,prefc,arefc,del_w,0);
