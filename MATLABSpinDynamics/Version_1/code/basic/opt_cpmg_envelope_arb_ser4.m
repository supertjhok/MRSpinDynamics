...
% ------------------------------------
% Allow arbitrary refocusing pulses, but fix nominal flip angles to 90
% degrees
% ------------------------------------
% If pvals is present as in input, only plot results

function [pvals,final_val]=opt_cpmg_envelope_arb_ser4(ne,T_E,optfun,a_init,pvals)
...
% Refocusing pulse (rectangular, 180 degrees)
%tref0=pi;
%pref0=pi/2;
%aref0=1;

% Refocusing pulse (rectangular, 135 degrees)
tref0=3*pi/4;
pref0=pi/2;
aref0=1;
...
if isempty(pvals) % Optimize
    ...
    if isempty(a_init)
        a_init=2*rand(1,ne); % Initial values
    end
    a_lb=zeros(1,ne); % Lower bounds
    a_ub=5*ones(1,ne); % Upper bounds
    ...
    for echonum=1:ne
        ...
        pvals(echonum)=fmincon(@(params)fit_function(params,texc,pexc,aexc,tref0,pref0,aref0,...
        ...
        tref0(2:nref-1)=tref0(2:nref-1)/pvals(echonum);
    ...
    for j=1:ne
        ind=(j-1)*nref+1;
        tref(1,ind+1:ind+nref-2)=tref0/pvals(j);
        pref(1,ind+1:ind+nref-2)=pref0;
        aref(1,ind+1:ind+nref-2)=aref0*pvals(j);
    end
...
nref=length(aref);
aref(2:nref-1)=aref(2:nref-1)*params;
tref(2:nref-1)=tref(2:nref-1)/params;
...
for n=1:ne
    ...
    figure(3);
    subplot(2,1,1);
    plot(n,eint(n),'bo'); hold on;
    plot(n,optfun(n),'r*'); 
    xlabel('Echo number');
    subplot(2,1,2);
    plot(del_w,abs(mecho(n,:)));
    xlim([-10 10])
    M(n)=getframe(gcf);
