...
function opt_exc_pulse_asymp_mag_repeat(nseg,tref,pref,aref,delt)
...
results=[];
while(1)
    [texc,pexc,echo_pk,echo_rms]=opt_exc_pulse_asymp_mag(nseg,neff,del_w,delt,[]);
    if count==1
        echo_rms_max=echo_rms;
        echo_pk_max=echo_pk;
        save results_mag_pk.mat texc pexc echo_pk echo_rms
        save results_mag_rms.mat texc pexc echo_pk echo_rms
    else
