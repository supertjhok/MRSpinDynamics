...
% ensembles with precalculated refocusing matrix
% Include calculation of single-pulse echo after excitation pulse
% ------------------------------------
...
function [spe,tvect_spe,echo,tvect]=sim_spin_dynamics_refmat_spe(T_90,tp,phi,tf,refmat)
...
% Sanity check
siz_refmat=size(refmat);
if siz_refmat(3)~=numpts
    disp('Error! Refocusing matrix must have the same size as the excitation matrix!');
    echo=0; tvect=0;
    return;
end
...
nspe=necho;
if delt>tf(end)-tmin
    delt=tf(end)-tmin; % Stop observing before next RF pulse
    nspe=round(delt/dt);
end
tvect_spe=linspace(tmin,delt+tmin,nspe);
...
% SPE
trans=zeros(1,numpts); % Only -1 coherence is visible
for k=1:numpts
    trans(k)=mvect(cind,1,k);
end

% Calculate time-domain waveform (SPE)
spe=zeros(1,nspe);
for j=1:nspe
    spe(j)=sum(trans.*exp(-i*del_w*tvect(j)))/numpts; % -1 coherence at time of detection
end

% Gap between excitation and fist refocusing pulse
mat=calc_fp_matrix_elements(del_w,tf(end)); % Free precession
for k=1:numpts
    mvect(:,:,k)=mat(:,:,k)*mvect(:,:,k);
end
