...
function [pvals,final_val]=opt_cpmg_envelope_arb_ser(ne,T_E,optfun,a_init)
...
for echonum=1:ne
    ...
    tmp=fmincon(@(params)fit_function(params,texc,pexc,aexc,tref0,pref0,aref0,...
        del_w,t_acq,masy,normasy,optfun,mvect1,mvect2,echonum),a_init0,[],[],[],[],a_lb0,a_ub0,[],options);
    pvals(echonum)=tmp(1);
...
function val=fit_function(params,texc,pexc,aexc,tref,pref,aref,del_w,...
    t_acq,masy,normasy,optfun,mvect1,mvect2,echonum)
...
eint=trapz(del_w,abs(mecho.*masy))/normasy;
val=sqrt((eint-normasy*optfun(echonum))^2);
