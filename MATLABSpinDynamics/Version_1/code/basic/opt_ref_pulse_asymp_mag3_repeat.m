...
function opt_ref_pulse_asymp_mag3_repeat(filname,pulse_nums,tacq)
...
for i=1:nums
    ...
    params.aref=results_orig{pulse_nums(i),3}; nref=(length(params.tref)-2)/2;
    params.tref=params.tref(2:nref+1); params.pref=params.pref(2:nref+1);
    params.aref=params.aref(2:nref+1);
    
    % Run optimization
    [out]=opt_ref_pulse_asymp_mag3(params);
    
    results{i,1}=[params.tfp out.tref fliplr(out.tref) params.tfp]; % Store refocusing cycle
    results{i,2}=[0 out.pref -fliplr(out.pref) 0];
    results{i,3}=[0 out.aref out.aref 0];
    results{i,4}=out.axis_rms;
    save(['results_ref_mag3_' num2str(nref) '.mat'], 'results');
