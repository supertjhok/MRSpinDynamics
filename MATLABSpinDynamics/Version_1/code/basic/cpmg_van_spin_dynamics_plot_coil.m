...
% NE = number of echoes
% TE = echo spacing
% Include effects of gating delays and tuned coil

% Delays follow pulses

function [echo,tvect,echo_pk,echo_rms]=cpmg_van_spin_dynamics_plot_coil(texc,tref,pexc,pref,T_90,NE,T_FP,T1,T2,T_gate,f_RF,Q)
...
tp(1:nexc)=texc;
phi(1:nexc)=pexc;
tf(1:nexc-1)=T_gate*ones(1,nexc-1);
...
for i=1:NE
    ...
    tf(nexc+(i-1)*nref+1:nexc+i*nref-1)=T_gate*ones(1,nref-1);
    tf(nexc+i*nref)=T_FP;
end
tf(nexc+NE*nref)=T_FP/2;

[echo,tvect]=sim_spin_dynamics_allpw_coil(T_90,tp,phi,tf,T1,T2,f_RF,Q);
