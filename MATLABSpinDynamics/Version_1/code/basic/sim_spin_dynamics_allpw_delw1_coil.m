...
% ensembles
% Include effect of RF inhomogeneity and tuned receive coil
% ------------------------------------
...
% f_RF, Q = center frequency & Q of tuned receive coil
...
function [echo,tvect]=sim_spin_dynamics_allpw_delw1_coil(T_90,del_w1,tp,phi,tf,T1,T2,f_RF,Q)
...
% Uniform distribution of del_w_RF (uniform gradient)
maxoffs=10;
...
w1=w1_max*(1+del_w1*(rand(1,numpts)-0.5)); % Random distribution

% B0 distribution
del_w=w1_max*cvec;

% Tuned coil transfer function
w_RF=2*pi*f_RF;
coil_tf=(i*(w_RF-del_w)/(w_RF*Q))./(1-((w_RF-del_w)/w_RF).^2+i*(w_RF-del_w)/(w_RF*Q));
...
for j=1:necho
    % w1 -> coil coupling / efficiency factor, coil_tf -> receiver frequency
    % response
    echo(j)=sum(trans.*exp(-i*del_w*tvect(j)).*w1.*coil_tf)/(numpts*w1_max); % -1 coherence at time of detection
end
