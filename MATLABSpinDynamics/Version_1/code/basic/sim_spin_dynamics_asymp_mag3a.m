...
function [minit,trans,masy]=sim_spin_dynamics_asymp_mag3a(tp,phi,amp,neff,del_w,t_acq)
...
mvect=zeros(3,numpts); minit=mvect; % Magnetization vectors
...
minit(1,:)=0.5*(mvect(3,:)+mvect(2,:)); % Mx
minit(2,:)=-0.5*1i*(mvect(3,:)-mvect(2,:)); % My
minit(3,:)=mvect(1,:); % Mz
...
trans=dot(minit,neff).*(neff(1,:)-1i*neff(2,:)); % (Mx - iMy) = M-
