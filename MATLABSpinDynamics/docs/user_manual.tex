...
add the active Version 2 code tree to the MATLAB path:
...
Use a recent MATLAB release for the current Version 2 scripts and graphics.
...
under \code{SpinDynamicsUpdated/Version\_2/code/mex}. Configure the compiler
with \code{mex -setup}. \\
\code{export\_fig} & Optional third-party File Exchange helper used by some
plotting/export scripts. Historical export paths may need local edits. \\
\bottomrule
\end{longtable}
...
Path & Purpose \\
\midrule
\path{SpinDynamicsUpdated/Version_2/code} & Recommended active MATLAB code. \\
\path{SpinDynamicsUpdated/Version_1} & Earlier updated implementation kept for
historical comparison. \\
\path{SpinDynamics} & Legacy original implementation. \\
...
\path{docs/VERSION_2_WORKFLOWS.md} & Workflow-oriented guide to Version 2 scripts. \\
...
These are not required for ordinary use. They require MATLAB Coder and a
configured compiler:
...
The sibling Python package uses this MATLAB Version 2 tree as its numerical
...
sometimes repeated across folders, and some scripts contain historical absolute
paths for figure export. Prefer the active Version 2 code tree for new work,
check \code{docs/VERSION\_GUIDE.md} when a function appears in more than one
place, and keep local path edits out of numerical changes.
