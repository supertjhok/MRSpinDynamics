...
function [out]=opt_exc_pulse_asymp_mag12(params)
...
nseg=length(texc);
texc=texc(1+delta:nseg); % Reduce number of segments by delta (eliminate from front)
pexc=pexc(1+delta:nseg);
%texc=texc(1:nseg-delta); % Reduce number of segments by delta (eliminate from end)
%pexc=pexc(1+nseg-delta);
start=pexc;
aexc=ones(1,nseg-delta); % Segments have arbitrary phase and constant amplitude

% Excitation pulse definition
% Use nonlinear function minimization - all segment times must be positive
lb=zeros(1,nseg-delta); % Lower bound
ub=2*pi*ones(1,nseg-delta); % Upper bound
