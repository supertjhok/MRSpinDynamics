...
function [ph,amp]=bloch_siegert_cpmg(expt,NE,T1,T2,bs_param)
...
[macq1,del_w]=sim_spin_dynamics_arba_relax_offs(tp,phi,amp,offs,acq,len_acq,T1,T2);

[tp,phi,amp,offs,acq]=create_bs_cpmg_timing(texc,pexc+pi,aexc,tref,pref,aref,NE,T_FP,bs_param);
[macq2,~]=sim_spin_dynamics_arba_relax_offs(tp,phi,amp,offs,acq,len_acq,T1,T2);
...
[tp,phi,amp,offs,acq]=create_bs_cpmg_timing(texc,pexc,aexc,tref,pref,aref,NE,T_FP,bs_param);
[macq5,~]=sim_spin_dynamics_arba_relax_offs(tp,phi,amp,offs,acq,len_acq,T1,T2);

[tp,phi,amp,offs,acq]=create_bs_cpmg_timing(texc,pexc+pi,aexc,tref,pref,aref,NE,T_FP,bs_param);
[macq6,~]=sim_spin_dynamics_arba_relax_offs(tp,phi,amp,offs,acq,len_acq,T1,T2);

[~,echo_bs1,~]=plot_function(macq1,macq2,del_w,tvect);
[~,echo_bs2,~]=plot_function(macq3,macq4,del_w,tvect);
[~,echo,~]=plot_function(macq5,macq6,del_w,tvect);
...
for i=1:NE
    ...
    tp(nexc+i*nref-1)=2*T_bs;
    tp(nexc+i*nref)=T_FP/2-2*T_bs;
...
%ph=atan(max(abs(imag(echo_bs)))/max(abs(real(echo_bs))));
%[~,ind]=min(abs(tvect));
%ph=atan(abs(imag(echo_bs(ind)))/abs(real(echo_bs(ind))));
