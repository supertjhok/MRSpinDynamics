...
% Use Rodrigues' rotation formula to speed up simulation (~3x) 10/04/11

function [macq,del_w]=sim_spin_dynamics_arb4(tp,phi,amp,offs,acq,len_acq,T1,T2)
...
del_w=linspace(-maxoffs,maxoffs,numpts); % Resonance offset distribution
numpts=length(del_w);
...
for j=1:num_pulses
    if amp(j)>0 % RF pulse
        dw=-del_w+offs(j);
        Omega=sqrt(amp(j).^2+dw.^2);
        w1=amp(j)./Omega; dw=dw./Omega;
