...
function [out]=opt_exc_pulse_asymp_mag13(params)
...
nref=length(tref);
if(nref == 5)
    tref(2)=tref(2)-delta*T_90; % Shorten reversed-phase segments
    tref(4)=tref(4)-delta*T_90;
    tref(3)=tref(3)+2*delta*T_90; % Lengthen central segment
    if(tref(2)<0 || tref(4)<0) % Use rectangular pulse
        tref=[tref(1) tref(3) tref(5)]; 
        pref=[pref(1) pref(3) pref(5)];
        aref=[aref(1) aref(3) aref(5)];
    end
else % Assume nref == 3
    tref(2)=tref(2)+2*delta*T_90; % Lengthen rectangular pulse
end
