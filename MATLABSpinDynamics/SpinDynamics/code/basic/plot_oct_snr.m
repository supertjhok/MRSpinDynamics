...
function [snr_norm,snr_matched_norm]=plot_oct_snr(filname)

% Simulation parameters
numpts=2001;  maxoffs=20;
del_w=linspace(-maxoffs,maxoffs,numpts);
tacq=5*pi; window = sinc(del_w*tacq/(2*pi)); % window function for acquisition
...
% Calculate SNR
snr_norm=zeros(1,siz(1)); snr_matched_norm = snr_norm; snr_max=0;
for i=1:siz(1)
    texc=results{i,1}; pexc=results{i,2}; aexc=ones(1,length(texc));
    tref=results{i,3}; pref=results{i,4}; aref=[0 ones(1,length(tref)-2) 0];
    ...
    snr_matched_norm(i)=trapz(del_w,abs(masy_matched).^2)/snr0;
