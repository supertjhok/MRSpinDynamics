function opt_exc_pulse_mag_results_te(expt)

T_90=pi/2;
len_acq=2*pi;

nvect=0.5:0.05:1.5; % Vector for te curve
npts=length(nvect);
...
for i=1:npts
    tref_curr=tref; % Modify free precession times
    tref_curr(1)=tref(1)*nvect(i);
    tref_curr(end)=tref(end)*nvect(i);
    
    [neff,del_w]=calc_rot_axis_arba(tref_curr,pref,aref);
    
    [masy1]=cpmg_van_spin_dynamics_asymp_mag(texc,pexc,aexc,neff,del_w,len_acq);
    if length(texc)==1e2  % OCT excitation pulse, use phase inversion instead of cycling
        [masy2]=cpmg_van_spin_dynamics_asymp_mag(texc,-pexc,aexc,neff,del_w,len_acq);
    else
        [masy2]=cpmg_van_spin_dynamics_asymp_mag(texc,pexc+pi,aexc,neff,del_w,len_acq);
    end
...
figure(5);
te_norm=(nvect*(tref(1)+tref(end))+sum(tref(2:end-1)))/(2*T_90);
plot(te_norm,echo_rms/echo_rms(ind),'bo-'); hold on;
xlabel('T_{E} / T_{180}');
