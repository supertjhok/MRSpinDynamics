...
function [macq,del_w]=sim_spin_dynamics_arba_relax_offs(tp,phi,amp,offs,acq,len_acq,T1,T2)
...
for j=1:num_pulses
    if amp(j)>0
        ...
        % Note: free precession is assumed to occur in current rotating frame, so use
        % del_w - offs(j) to calculate the matrix
        mat=calc_fp_matrix_elements(del_w-offs(j),tp(j),T1,T2); % Free precession
