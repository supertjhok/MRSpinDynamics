...
function [n]=calc_rot_axis_arba2(tp,phi,amp,del_w,plt)

numpts=length(del_w);
%zero_tol=1e-12;

% Calculate rotational axes (no relaxation or diffusion), [x y z] form
n=zeros(3,numpts); ncurr=n; tmp2=n; % Rotation axes
...
if amp(1)>0 % RF pulse
    ...
    alpha=Omega*tp(1);
    n(1,:)=w1*cos(phi(1))./Omega;
    n(2,:)=w1*sin(phi(1))./Omega;
    n(3,:)=del_w./Omega;
else % Free precession
    alpha=del_w*tp(1);
    n(3,:)=ones(1,numpts); % Initial rotation axis is the z-axis
end
sn=sin(alpha/2); cs=cos(alpha/2);
...
for j=2:num_pulses
    ...
    if amp(j)>0
        ...
        tmp1=cs.*cs_c-sn.*sn_c.*dot(n,ncurr);
        tmp2(1,:)=sn.*cs_c.*n(1,:)+cs.*sn_c.*ncurr(1,:)-sn.*sn_c.*crs(1,:);
        tmp2(2,:)=sn.*cs_c.*n(2,:)+cs.*sn_c.*ncurr(2,:)-sn.*sn_c.*crs(2,:);
        tmp2(3,:)=sn.*cs_c.*n(3,:)+cs.*sn_c.*ncurr(3,:)-sn.*sn_c.*crs(3,:);
        ...
        tmp1=cs.*cs_c-sn.*sn_c.*n(3,:);
        tmp2(1,:)=sn.*(cs_c.*n(1,:)-sn_c.*n(2,:));
        tmp2(2,:)=sn.*(cs_c.*n(2,:)+sn_c.*n(1,:));
        tmp2(3,:)=sn.*cs_c.*n(3,:)+cs.*sn_c;
    end
    
    cs=tmp1; alpha=2*acos(cs); sn=sin(alpha/2);
