...
% ensembles
% Include effect of tuned receive coil
% ------------------------------------
...
function [echo,tvect]=sim_spin_dynamics_allpw_coil(T_90,tp,phi,tf,T1,T2,f_RF,Q)
...
for j=1:necho
    echo(j)=sum(trans.*exp(-i*del_w*tvect(j)).*coil_tf)/numpts; % -1 coherence at time of detection
end
