...
function [out]=opt_exc_pulse_asymp_mag17(params)
...
nseg=length(texc);
if ntarget>nseg % Make pulse longer
    nseg=nseg+1;
    texc=[texc(1) texc]; 
    start=[2*pi*rand(1) pexc];
else % Make pulse shorter
    nseg=nseg-1;
    texc=texc(1:nseg); 
    start=pexc(1:nseg);
end
...
options=optimset('Algorithm','interior-point','Display','iter','TolFun',3e-4,'MaxFunEvals',2e4);
