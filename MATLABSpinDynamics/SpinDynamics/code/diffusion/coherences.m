...
asy;       % calculates the asymptotic echo shape for a CPMG sequence in a gradient field without diffusion.
Lambdas;   % calculates the transition elements between the difference coherences for a nominal 180 pulse 
           % and for the initial 90 pulse
 
Nfinal = 10;  % The higher the echo number, the longer the calculation will be. Nfinal = 10 is just a few seconds, 
...
% (Necho +1)th echo. 
  ...
  disttime = zeros(1,3*64);
  axistime = 10.^(-(-1:length(disttime)-2)./64);
  msum = zeros(1,length(omega0));
  numberpath = 0;
  normasy    = sum(masy.*masy);
  ...
  for Ntrans = 0:Necho  % change number of full periods 
       ...
       marbles;   % calculate all different ways to distribute the Ntrans
                  % periods among the Necho periods
       pointertrans = pointerplus;
       itrans       = imax;
       ...
       for ipath = 1:imax*itrans
           ...
           % calculate the spectrum of the sum of all path ways (should approach masy)
             msum   = mpathy + msum;
           ...
           % normalized decay time w.r.t. to on resonance
             ...
             disttime(indextime) = disttime(indextime) + wpath;
           % hold on
...
%     
%     subplot(2,1,2)
%       semilogx(axistime,disttime)
%       axis([0.8e-3 1.2 -0.2 1])
      
      disttimes(Necho+1,:) = disttime;
      msums(Necho+1,:) = msum;
...
for iecho = Nfinal-2:Nfinal
 ...
 figure(101)
       bar(1./axistime,disttimes(Nfinal,:))
       set(gca,'xscale','log')
       axisold =axis;
       axis([0.5 250 axisold(3:4)])
       xlabel('\eta_l^{(N)}')
       ylabel('a_l^{(N)}')
       text(50,0.2,['N = ',num2str(Nfinal)])