...
% Plotting parameters
% --------------------------------------------
sp.plt_tx = 1; sp.plt_rx = 1; sp.plt_sequence = 1; % Plot on/off
sp.plt_axis = 1;  sp.plt_mn = 1; sp.plt_echo = 1;

% Pulse sequence parameters
% --------------------------------------------
pp.N=32; % quantization step = N x input RF frequency
pp.T_90=25e-6; pp.T_180=2*pp.T_90; % Rectangular T_90 and T_180

pp.psi=0; % Absolute RF phase at t=0

% Excitation pulse
pp.texc=[1]*pp.T_90; pp.pexc=[pi/2]; pp.aexc=[1];
pp.tcorr=-(2/pi)*pp.T_90; % Timing correction for excitation pulse

% Refocusing cycle
pp.tref=[3 1 3]*pp.T_180; pp.pref=[0 0 0]; pp.aref=[0 1 0];
