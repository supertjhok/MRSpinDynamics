...
| `spin_dynamics.workflows.pgse` | Stejskal-Tanner PGSE theory and moving-isochromat extension | b-value equivalence, `exp(-bD)` attenuation, zero-gradient/zero-diffusion limits, backend dispatch, and walker convergence against the moment backend | Passed |
...
```text
Added rectangular PGSE and PGSE-prepared CPMG workflows with two backends:
deterministic gradient-moment tracking and explicit random walkers. The moment
backend is checked against the Stejskal-Tanner b-value and `exp(-bD)`
attenuation; the walker backend is checked statistically against the moment
backend through a zero-diffusion reference. Added a PGSE D-T2 plotting example
that builds b-values from the sequence workflow and recovers a D-T2 map with
SciPy-backed non-negative ILT.
Result: OK (PGSE tests: 6 passed; diffusion/motion/PGSE focused tier:
21 passed; PGSE D-T2 example rendered with SciPy NNLS)
```
