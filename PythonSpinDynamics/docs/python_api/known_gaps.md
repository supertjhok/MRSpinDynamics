...
- multi-pulse ESR beyond the rectangular-pulse FID/Hahn-echo and four-pulse
  DEER helpers (ESEEM and HYSCORE are not yet implemented); anisotropic
  hyperfine and exchange couplings; finite/shaped DEER pump pulses and explicit
  excitation-bandwidth selection (the DEER model uses ideal spin-selective
  pulses and an explicit modulation depth); higher-spin zero-field splitting;
  temperature-dependent equilibrium magnetization; and ESR saturation or
  resonator models;
