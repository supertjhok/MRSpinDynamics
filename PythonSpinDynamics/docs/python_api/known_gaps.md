...
- ideal no-probe v0crit and time-varying-field refocusing evaluation, bounded
  optimizers, and multi-start drivers;
...
- array-returning multi-start optimization driver scaffolds for repeated
  ...
  searches;
- MATLAB-style optimization result-cell conversion plus `.npz` export and
  optional SciPy-backed `.mat` export;
...
- broad diffusion sweeps, Q>2000 diffusion validation, and additional
  diffusion variants;
- full script-specific MATLAB `.mat` result parity, broad `fmincon` parity
...
The next natural migration targets are full script-specific MATLAB result-file
parity, broader optimization parity validation, and stronger
inverse-excitation workflows, with additional diffusion validation as a
parallel track.
